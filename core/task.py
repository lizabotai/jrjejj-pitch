...
def create_daily(token, slug, proof, proxies=None):
...
def process_do_task(token, proxies=None):
    ...
    for daily_task in daily_tasks:
        ...
        do_daily_task = create_daily(
            token=token, slug=slug, proof=proof, proxies=proxies
        )
        try:
...
def claim_referral(token, proxies=None):
...
def process_claim_ref(token, proxies=None):
    referral_rewards = get_info(token=token, proxies=proxies)["referralRewards"]
    if referral_rewards > 0:
        start_claim_ref = claim_referral(token=token, proxies=proxies)
